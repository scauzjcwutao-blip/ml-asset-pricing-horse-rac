...
Python Replication Script - Real Data Ready Version
Author: Tao Wu
Date: May 2026

This script supports real WRDS data + interactive CSV loading.
"""
...
def load_data():
    """Interactive data loader: support local CSV (WRDS export) or demo data."""
    print("\n🔄 Data Loading Mode")
    print("   Press Enter to use demo data, or enter full path to CSV files (WRDS format).")

    # Fund returns
    fund_path = input("   Enter path to fund_returns.csv (or Enter for demo): ").strip()
    if fund_path:
        fund_returns = pd.read_csv(fund_path, index_col=0, parse_dates=True)['fund_ret']
        print(f"✅ Loaded real fund_returns: {fund_returns.shape}")
    else:
        # Demo data
        ...
        print("✅ Using demo fund_returns")

    # 13F Holdings
    holdings_path = input("   Enter path to holdings.csv (or Enter for demo): ").strip()
    if holdings_path:
    ...
    # Stock returns (for virtual portfolio)
    stock_path = input("   Enter path to stock_returns.csv (or Enter for demo): ").strip()
    if stock_path:
    ...
    # Factors (for risk-adjusted alpha)
    factor_path = input("   Enter path to factors.csv (or Enter for demo): ").strip()
    if factor_path:
        factors = pd.read_csv(factor_path, index_col=0, parse_dates=True)
        print(f"✅ Loaded real factors: {factors.shape}")
    else:
        # Demo Carhart 4 factors
        quarters = pd.date_range(start='1994-03-31', end='2019-12-31', freq='QE')
