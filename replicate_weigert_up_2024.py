...
"""
Unobserved Performance of Hedge Funds (Weigert et al., Journal of Finance 2024)
Python Replication Script - Corrected Full Version (Multi-Fund Cross-Sectional Sort)

Author: Tao Wu (Corrected)
Date: May 2026

Key corrections vs. previous version:
  1. All funds processed as a panel (not single fund)
  2. Monthly returns compounded to quarterly
  3. Cross-sectional quintile sorting each quarter
  4. Next-quarter fund returns used as portfolio return (predictive sort)
  5. Newey-West (HAC) standard errors for time-series regression
  6. Proper alignment between holdings lag and return periods

CSV FILE FORMAT REQUIREMENTS:
  1. fund_returns.csv
     - Columns: 'date', 'fund_id', 'fund_ret' (decimal, monthly)
     - Example: 1994-01-31, FUND001, 0.0123

  2. holdings.csv
     - Columns: 'report_date', 'fund_id', 'stock', 'weight'
     - report_date should be quarter-end dates (13F filing dates)

  3. stock_returns.csv
     - Columns: 'date', 'stock', 'ret' (decimal, monthly)
     - Or wide format: index=date, columns=stock tickers

  4. factors.csv
     - Columns: 'date', 'MKT', 'SMB', 'HML', 'MOM' (decimal, monthly)
"""
...
print("Corrected Version: Multi-Fund Panel + Cross-Sectional Quintile Sort")
print("=" * 80)
...
def generate_demo_data(n_funds=100, n_stocks=50, start='1994-01-31', end='2019-12-31'):
    """
    Generate realistic demo data for multiple funds.
    In demo mode we simulate:
      - 100 hedge funds with monthly returns
      - 50 stocks with monthly returns
      - Quarterly 13F holdings for each fund
      - Fama-French + MOM monthly factors
    """
    ...
    # Simulate with some cross-sectional dispersion and persistence
    fund_records = []
    for fid in fund_ids:
        # Each fund has a slightly different alpha and volatility
        fund_alpha = np.random.normal(0.005, 0.003)  # monthly alpha ~0.5% avg
        fund_vol = np.random.uniform(0.02, 0.06)
        rets = np.random.normal(fund_alpha, fund_vol, len(months))
    ...
    print(f"   ✅ Fund returns: {len(fund_returns_df)} rows ({n_funds} funds × {len(months)} months)")

    # --- Stock monthly returns ---
    stock_records = []
    for sid in stock_ids:
        stock_mu = np.random.normal(0.008, 0.005)
        stock_vol = np.random.uniform(0.04, 0.12)
        rets = np.random.normal(stock_mu, stock_vol, len(months))
        for date, ret in zip(months, rets):
            stock_records.append({'date': date, 'stock': sid, 'ret': ret})
    stock_returns_df = pd.DataFrame(stock_records)
    print(f"   ✅ Stock returns: {len(stock_returns_df)} rows ({n_stocks} stocks × {len(months)} months)")

    # --- 13F Holdings (quarterly, per fund) ---
    holdings_records = []
    for fid in fund_ids:
        # Each fund holds 5-20 stocks, rebalanced each quarter with some persistence
        n_hold = np.random.randint(5, 20)
        current_stocks = np.random.choice(stock_ids, n_hold, replace=False)
        for q in quarters:
            # Slight turnover each quarter
            if np.random.random() < 0.3:  # 30% chance of full reshuffle
                n_hold = np.random.randint(5, 20)
                current_stocks = np.random.choice(stock_ids, n_hold, replace=False)
            else:
                # Small changes
                n_change = max(1, n_hold // 5)
                drop_idx = np.random.choice(len(current_stocks), min(n_change, len(current_stocks)), replace=False)
                keep = np.delete(current_stocks, drop_idx)
                new_stocks = np.random.choice(
                    [s for s in stock_ids if s not in keep],
                    min(n_change, n_stocks - len(keep)), replace=False
                )
                current_stocks = np.concatenate([keep, new_stocks])
    ...
    # --- Fama-French + MOM factors (monthly) ---
    ...
    print(f"   ✅ Factors: {len(factors_df)} rows (monthly)")

    return fund_returns_df, holdings_df, stock_returns_df, factors_df


def load_real_data():
    """Load real CSV data (WRDS format)."""
    ...
    n_funds = fund_returns_df['fund_id'].nunique()
    print(f"   ✅ Fund returns: {len(fund_returns_df)} rows, {n_funds} funds")
    ...
    print(f"   ✅ Stock returns: {len(stock_returns_df)} rows")
...
def construct_buy_and_hold_return(fund_id, quarter, holdings_df, stock_ret_wide):
    """
    Compute the buy-and-hold return of the PREVIOUS quarter's 13F holdings
    held passively through the current quarter.

    Logic:
      - Look up holdings reported BEFORE `quarter`
      - Use the most recent report_date < quarter
      - Compute weighted average of stock returns in `quarter`
    """
    prev_reports = holdings_df[
        (holdings_df['fund_id'] == fund_id) &
        (holdings_df['report_date'] < quarter)
    ]
    ...
    # Get stock returns for this quarter
    if quarter not in stock_ret_wide.index:
        return np.nan

    stock_rets = stock_ret_wide.loc[quarter]
    common_stocks = prev_holdings['stock'].values
    common_stocks = [s for s in common_stocks if s in stock_rets.index and not pd.isna(stock_rets[s])]

    if len(common_stocks) == 0:
        return np.nan

    weights = prev_holdings.set_index('stock').loc[common_stocks, 'weight']
    # Re-normalize after dropping missing stocks
    weights = weights / weights.sum()
    returns = stock_rets[common_stocks]

    bh_return = (weights * returns).sum()
    return bh_return


def calculate_up_panel(fund_quarterly, holdings_df, stock_ret_wide):
    """
    Calculate Unobserved Performance (UP) for all funds, all quarters.

    UP(i,t) = Reported_Return(i,t) - BuyAndHold_Return(i,t)

    Returns a DataFrame with columns: ['quarter', 'fund_id', 'UP', 'fund_ret_q']
    """
    ...
    for idx, fund_id in enumerate(fund_ids):
        ...
        for _, row in fund_data.iterrows():
            ...
            bh_ret = construct_buy_and_hold_return(fund_id, q, holdings_df, stock_ret_wide)
    ...
    for q_bin in range(1, 6):
        if port_returns[q_bin]:
            ...
            print(f"   Q{q_bin} {'(Low UP)' if q_bin == 1 else '(High UP)' if q_bin == 5 else '':<8}"
                  f" {mean_q:>14.4f} {ann_ret:>10.2f}% {std_q:>10.4f} {sharpe:>14.2f}")
    ...
    for q_bin in range(1, 6):
        if port_returns[q_bin]:
            ...
            if len(combined) >= 12:
                ...
                print(f"   Q{q_bin:<11} {alpha:>12.4f} {alpha * 4:>12.2f}% {t:>10.2f}")
...
if __name__ == "__main__":
    ...
    summary = pd.DataFrame({
        'Metric': [
            'Number of funds',
            'Number of quarters',
            'UP panel observations',
            'LS portfolio quarters',
            'Mean LS return (quarterly)',
            'Alpha (quarterly, Newey-West)',
            'Alpha t-stat',
            'Alpha (annualized)',
        ],
        'Value': [
            up_panel['fund_id'].nunique(),
            up_panel['quarter'].nunique(),
            len(up_panel),
            len(ls_df),
            f"{ls_df['LS_return'].mean():.4f}",
            f"{alpha_results['alpha_quarterly']:.4f}" if alpha_results else "N/A",
            f"{alpha_results['t_stat']:.2f}" if alpha_results else "N/A",
            f"{alpha_results['alpha_annual']:.4f}" if alpha_results else "N/A",
        ]
    })
    ...
    print(f"   - up_panel.csv          : Full UP panel (fund × quarter)")
    print(f"   - long_short_returns.csv : Q5-Q1 long-short time series")
    print(f"   - summary.csv           : Key statistics")
    print("\n📝 Notes:")
    print("   - Demo data uses random returns → no real economic signal expected")
    print("   - With real WRDS data, expect UP ~ 1-3% annualized alpha (per paper)")
    print("   - Paper reference: Weigert et al., Journal of Finance, 2024")
    print("=" * 80)
2. holdings.csv
